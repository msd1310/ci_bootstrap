...
*/
		...
		if($this->input->post()){
			...
			if(!empty($this->input->post('fromtime')) && !empty($this->input->post('totime')) ){
                    ...
                    $report_data = $this->report_model->select_estimate_data_time($ptype,$fdate,$tdate);
                     $this->mViewData['report_data'] = $report_data;
                     if($this->input->post('hdnAction') == 'excel'){    
                                $excel_data = $this->report_model->select_estimate_data_time_excel($ptype,$fdate,$tdate);
                                //print_r($excel_data);
                                $this->ExportCSV_estimate2($excel_data); 
                     }
                }
                else{ 
				...
				//$this->mViewData['report_data'] = $this->report_model->select_estimate_data($ptype,$fdate,$tdate,$stype);
                     if($this->input->post('hdnAction') == 'excel'){    
                            $excel_data = $this->report_model->select_estimate_data_excel($ptype,$fdate,$tdate,$stype);
                              $this->ExportCSV_estimate2($excel_data); 
                     }
			}
		}else{
		...
		$data_array = array('ptype'=>$ptype,'fdate'=>date('Y-m-d',strtotime($fdate)),
                        'tdate'=>date('Y-m-d',strtotime($tdate)),'ftime'=>$ftime,'ttime'=>$ttime);
        $this->mViewData['data']= $data_array;
		
	}

	function collection_print($data) {
                ...
                if($this->input->post()){
						...
						elseif($this->input->post('hdnAction') == "print"){
							 if($fdate == '' and $tdate == ''){
                                       ...
                                       $data = $this->report_model->select_collection_excel_data($ptype,$fdate,$tdate,$stype);
                                       $this->collection_print($excel_data);
                             }
...
$this->load->library('excel');
    ...
    $objPHPExcel->setActiveSheetIndex(0);
     //$objPHPExcel->getActiveSheet()->setTitle('Countries');
                ...
                $objPHPExcel->getActiveSheet()->getStyle('A3:H3')->getFill()->getStartColor()->setARGB('21bc04');
                ...
                $objPHPExcel->getActiveSheet()->setCellValue('B3', 'Vehicle Type');
                $objPHPExcel->getActiveSheet()->setCellValue('C3', 'Vehicle Number');
                $objPHPExcel->getActiveSheet()->setCellValue('D3', 'Container No.');
                $objPHPExcel->getActiveSheet()->setCellValue('E3', 'In-time');
                $objPHPExcel->getActiveSheet()->setCellValue('F3', 'Out-time');
                //merge cell A1 until C1
                //$objPHPExcel->getActiveSheet()->mergeCells('D3:E3');
                $objPHPExcel->getActiveSheet()->setCellValue('G3', 'Total Hours');
                $objPHPExcel->getActiveSheet()->setCellValue('H3', 'Charges');
                //set aligment to center for that merged cell (A1 to C1)
   ...
   /***********average hours calculation*********/
    ...
    $ave_sec = (int) $hours .':'.(int) $minutes.':'.(int) $seconds;/*****************/
                ...
                $exceldata[] = array($pid,$vtype,$noplate,$containerno,$intime,$row['outtime'],$new_hours,$total_charges,$status);
                $charges = $total_charges;
                $charges_t = $charges_t + $charges; 
                $ii = 1;
                $objPHPExcel->getActiveSheet()->getStyle('A'.$ii.':H'.$ii)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_LEFT);
    ...
    $objWriter->save('php://output');
      ...
      public function ExportCSV_estimate2($data){
   ...
   /***********average hours calculation*********/
    ...
    $ave_sec = (int) $hours .':'.(int) $minutes.':'.(int) $seconds;/*****************/
                ...
                $exceldata[] = array('','TOTAl :'.count($data->result_array()),'','','','','Total Charges  :',$charges_t,'');
                //$exceldata[] = array('','','','','','average hours :'.$ave_sec,'','');
                $excel_c = count($data->result_array()) +4;
    ...
    // Sending headers to force the user to download the file
    header('Content-Type: application/vnd.ms-excel');
    header('Content-Disposition: attachment;filename="Estimate'.date('y-m-d').'.xls"');
    header('Cache-Control: max-age=0');
