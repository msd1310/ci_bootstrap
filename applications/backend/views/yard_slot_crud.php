...
<h3> Choose slots by clicking the corresponding slots in the layout below:</h3>
  ...
  <tr>
    ...
    <td><dt class="grey">&nbsp;</dt></td><td><dd>- Slots alloted to selected yard</dd></td>
  </tr>
</tbody></table>
...
//alert(slot_data[0].name);
            ...
            var settings = {
               ...
               yardSeatCss: 'yardSeat',
               selectingSeatCss: 'selectingSeat'
           };


          var init = function (reservedSeat,bookedSeat) {
               ...
               for (i = 0; i < rows_s; i++) {
                    for (j = 0; j < cols_s; j++) {
                        ...
                        if ($.isArray(bookedSeat) && $.inArray(seatNo, bookedSeat) != -1) {
                            className += ' ' + settings.yardSeatCss;
                        }
          ...
          var booked ;
                $.ajax({
                  ...
                  success: function (data) {
                           //alert(data.booked);
                           init(data.booked,null);
                            //alert('This slot is already alloted ');
...
$( "#yard" ).change(function() {
                          $.ajax({
                                                    url:"<?php echo site_url('plaza/getYardSlotDetailsByYard');?>",
                                                    type: 'POST',
                                                    dataType: "json",
                                                    data:{'yard_id': this.value },
                                                    success: function (data) {
                                                             init(data.total,data.booked);

                                                                $('.' + settings.seatCss).click(function () {
                                                                if ($(this).hasClass(settings.selectedSeatCss)){
                                                                    alert('This slot is already alloted to another yards');
                                                                } else if ($(this).hasClass(settings.yardSeatCss)){
                                                                    alert('This slot is already alloted to selected yard');
                                                                }
                                                                else{
                                                                    $(this).toggleClass(settings.selectingSeatCss);
                                                                    }
      ...
      $("#btn_slot").click(function(){
        ...
        item = $(this).attr('title');                   
         ...
         //alert(slots);
             $.ajax({
                  url:"<?php echo site_url('yard_slot/saveYardSlot');?>",
                  type: 'POST',
                  data:{'yard_id':$('#yard').val(),'slotArray':slots},
                  dataType: "json",
                  success: function (data) {
                        alert("Slots Assigned Succesfully");
                  }   
              });
              location.href = "<?php echo site_url('yard_slot');?>";
      });
